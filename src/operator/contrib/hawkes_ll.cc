...
NNVM_REGISTER_OP(_contrib_hawkesll)
    .describe(R"code(Computes the log likelihood of a univariate Hawkes process [1]_.
...
.. math::
  ...
  \lambda^*(t) = \lambda_k + \alpha_k \sum_{\{t_i < t, y_i = k\}} \beta_k \exp(-\beta_k (t - t_i))
...
References:
  .. [1] Bacry, E., Mastromatteo, I., & Muzy, J. F. (2015). \
    ...
    .set_attr<nnvm::FListInputNames>("FListInputNames",
        ...
        return std::vector<std::string>{"lda", "alpha", "beta", "state", "lags", "marks", "valid_length", "max_time"};
    })
    ...
    .add_argument("lda", "NDArray-or-Symbol", "The intensity for each of the K processes")
    .add_argument("alpha", "NDArray-or-Symbol", "The infectivity factor (branching ratio) for each process")
    .add_argument("beta", "NDArray-or-Symbol", "The decay tail exponent for each process")
    ...
    .add_argument("valid_length", "NDArray-or-Symbol",
      "The number of valid points in the process")
    .add_argument("max_time", "NDArray-or-Symbol", "the length of the interval where the processes were sampled")
;
